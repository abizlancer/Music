...
export default {
  install(app) {
    ...
    configure({
      generateMessage: (ctx) => {
        const messages = {
          required: `The field is ${ctx.field} is required.`,
          min: `The field ${ctx.field} is too short.`,
          max: `The field ${ctx.field} is too long.`,
          alpha_spaces: `The field ${ctx.field} may onlu contain alphabetical characcters and spaces.`,
          ...
          tos: "You must exept the Terms of Service.",
        };
        
        const message = messages[ctx.rule.name]
          ? messages[ctx.rule.name]
          : `The field ${ctx.field} is valid.`;
        return message
      },
      validateOnBlur: true,
